...
### 分层架构
```
CLI层 (表现层) - CommandRouter统一路由(57个命令)
    ↓
Facade层 (外观层) - RbacFacade粗粒度业务编排
    ↓
Service层 (业务层) - 7个Service + 3个DTO
    ↓
Repository层 (持久层) - 7个Repository + 范围权限查询
    ↓
Database层 (MySQL) - 8个核心表
```

### 核心设计模式
- **外观模式** - 简化CLI与多个Service的交互
- **命令模式** - CommandRouter统一管理命令
- **模板方法** - 统一业务执行流程
- **策略模式** - 支持多种密码加密算法
- **单例模式** - 数据库连接管理
...
├── cli/              # CLI层
│   ├── handler/      # CommandRouter统一命令路由(57个命令)
...
├── exception/        # 异常体系
├── facade/           # 外观层
├── repository/       # 持久层(7个Repo + 范围权限查询)
├── security/         # 安全组件(密码加密、数据脱敏)
├── service/          # 业务服务层(7个Service + 3个DTO)
└── Main.java         # 程序入口
```
...
- [x] 修改用户(update-user)
- [x] 删除用户(delete-user)
- [x] 分配角色给用户(assign-role)
- [x] 移除用户角色(remove-role)
...
### 权限管理
- [x] 创建权限(create-permission)
- [x] 查看权限列表(list-permissions)
- [x] 查看我的权限(list-my-permissions)
- [x] 修改权限(update-permission)
- [x] 删除权限(delete-permission)
- [x] 分配权限给角色(assign-permission)
- [x] 移除角色权限(remove-permission)
- [x] 分配资源权限(范围控制)(assign-resource-permission)
- [x] 移除资源权限(remove-resource-permission)

### 资源管理
- [x] 创建资源(create-resource)
- [x] 查看资源列表(list-resources)
- [x] 查看我的资源(list-my-resources)
- [x] 查看资源详情(view-resource)
- [x] 修改资源(update-resource)
- [x] 删除资源(delete-resource)

### 审计日志
- [x] 查看自己的操作日志(view-audit)
- [x] 查看所有审计日志(view-all-audit)
- [x] 按用户查询日志(view-user-audit)
- [x] 按操作类型查询(view-action-audit)
- [x] 按资源查询日志(view-resource-audit)

### 系统功能
- [x] 用户登录/登出(login/logout)
- [x] 查看个人资料(view-profile)
- [x] 修改密码(change-password)
- [x] 修改个人信息(change-profile)
- [x] 自动初始化默认数据
- [x] help命令查看可用命令
- [x] 权限驱动命令显示
